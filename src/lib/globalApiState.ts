...
export function shouldFetchMenuItems(): boolean {
  ...
  // Bloquear cualquier fetch por 2 segundos después del primero
  if (timeSinceLastFetch < 2000 && !globalApiState.menuItemsFetched) {
...
export function shouldFetchOrders(): boolean {
  ...
  // Bloquear cualquier fetch por 2 segundos después del primero
  if (timeSinceLastFetch < 2000 && !globalApiState.ordersFetched) {
...
export function shouldFetchReservations(): boolean {
  ...
  // Bloquear cualquier fetch por 2 segundos después del primero
  if (timeSinceLastFetch < 2000 && !globalApiState.reservationsFetched) {
